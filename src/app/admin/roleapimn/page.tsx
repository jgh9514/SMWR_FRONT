...
export default function RoleApiManagementPage() {
  ...
  const apiHeaders = useMemo(() => {
    const cols = [
      ...
      { title: '업무구분', key: 'bsns_cd_nm', align: 'center' as const },
    ...
    <Box sx={{ minHeight: '100vh', bgcolor: 'background.default', py: { xs: 2, md: 4 } }}>
      <Container maxWidth="xl">
        ...
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 2 }}>
          ...
          {/* 권한 API */}
          <Box sx={{ flex: { xs: '1 1 100%', md: '1 1 calc(75% - 8px)' } }}>
            <Card>
              <CardHeader title="권한 API" />
              <CardContent>
                {selectedRoleId ? (
                  <>
                    <TableContainer>
                        ...
                        <TableBody>
                          {apiList.length === 0 ? (
                            ...
                            apiList.map((row) => {
                              ...
                              return (
                                <TableRow key={row.api_id} hover>
                                  <TableCell padding="checkbox">
                                    <Checkbox
                                      checked={isSelected}
                                      onChange={() => toggleSelectApi(row.api_id)}
                                    />
                                  </TableCell>
                                  {!mobile && <TableCell align="center">{row.api_id}</TableCell>}
                                  {!mobile && <TableCell align="center">{row.bsns_cd_nm}</TableCell>}
