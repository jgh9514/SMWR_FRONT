...
export default function BatchManagementPage() {
  ...
  const handleViewResult = useCallback(
    async (runSn: string | number, previewText?: string) => {
      ...
      setSelectedResult(previewText ? `${previewText}\n\n(전문 로딩 중…)` : '로딩 중…');
      try {
        const detail = await historyDetailMutation.mutateAsync({ runSn });
        const full = detail?.rslt_txt?.trim();
        setSelectedResult(full || previewText?.trim() || '결과가 없습니다.');
      } catch (e) {
        logger.error('[batch] run-his detail failed', { runSn, error: e });
        setSelectedResult(previewText?.trim() || '로그를 불러오지 못했습니다.');
        showToast.error('배치 로그 상세를 불러오지 못했습니다.');
      }
    },
    [historyDetailMutation],
  );
  ...
  const panelTableHeight = { xs: 360, md: 'calc(100vh - 240px)' };
      ...
      <Container maxWidth="xl" sx={{ py: { xs: 2, md: 3 } }}>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap', minWidth: 0 }}>
            ...
            {incident && <Chip label={`incident: ${incident}`} color="warning" variant="outlined" size="small" />}
            {historyFilter === 'failed' && (
              <Chip label="실패·실행중 필터" color="error" variant="outlined" size="small" />
            )}
          </Box>
          <Tooltip title="설정·이력 새로고침">
            <span>
              <Button
                variant="outlined"
                size="small"
                ...
                disabled={isLoadingConfig || isLoadingHistory}
              >
                새로고침
              </Button>
            </span>
        ...
        >
                ...
                >
                  <Table size="small" stickyHeader>
                    <TableHead>
                      <TableRow>
                        {[
                          { label: 'ID', sx: { width: 56 }, hide: false },
                          { label: '배치명', sx: { minWidth: 100 }, hide: false },
                          { label: '스케줄', sx: { minWidth: 88 }, hide: mobile },
                          { label: '상태', sx: { width: 64 }, align: 'center' as const, hide: false },
                          { label: '최근', sx: { minWidth: 88 }, hide: false },
                          { label: '', sx: { width: 72 }, align: 'center' as const, hide: false },
                        ]
                          .filter(({ hide }) => !hide)
                          .map(({ label, sx, align }) => (
                          <TableCell
                    ...
                    <TableBody>
                      {sortedBatchConfigList.map((config) => {
                        ...
                        >
                          ...
                          {!mobile && (
                            <TableCell>
                              <Tooltip title={config.cron_expr}>
          ...
          >
            <CardHeader
              ...
              subheader={
                selectedBatId
                  ? `${selectedBatchName ?? selectedBatId} · 최근 ${historyQueryParams.limit}건`
                  : `전체 · 표시 ${filteredBatchHistory.length}건`
              }
              titleTypographyProps={{ variant: 'subtitle1', fontWeight: 700 }}
              subheaderTypographyProps={{ variant: 'caption' }}
              sx={{ py: 1.5, px: 2, borderBottom: 1, borderColor: 'divider' }}
            ...
            >
              ...
              >
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, alignItems: 'center' }}>
                  <FormControl size="small" sx={{ flex: 1, minWidth: 140 }}>
                    <InputLabel id="batch-history-filter-label">배치</InputLabel>
                    <Select
                      labelId="batch-history-filter-label"
                      label="배치"
                      value={selectedBatId ?? ''}
                      MenuProps={MUI_MENU_A11Y_PROPS}
                      onChange={(e) => {
                        blurFocusedMenuItem();
                  ...
                  {selectedBatId && (
                    <Tooltip title="필터 해제">
                      <IconButton size="small" onClick={handleClearBatchFilter} aria-label="필터 해제">
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75 }}>
                  {([
                    ...
                    ['running', '진행', batchHistorySummary.runningCount],
              ...
              {historyFilter === 'failed' && (
                <Alert severity="info" sx={{ mx: 2, mt: 1, py: 0.5 }} icon={false}>
                  <Typography variant="caption">실패·실행중 이력만 표시 중</Typography>
                </Alert>
              )}
              {isLoadingHistory ? (
                <Box sx={{ display: 'flex', justifyContent: 'center', py: 4 }}>
                ...
                >
                  <Table size="small" stickyHeader>
                    <TableHead>
                      <TableRow>
                        {[
                          { label: 'ID', sx: { width: 52 }, hide: mobile },
                          { label: '배치', sx: { minWidth: 72 }, hide: !!selectedBatId },
                          { label: '상태', sx: { width: 72 }, align: 'center' as const, hide: false },
                          { label: '시작', sx: { minWidth: 120 }, hide: false },
                          { label: '소요', sx: { width: 64 }, align: 'center' as const, hide: false },
                          { label: '로그', sx: { width: 72 }, align: 'center' as const, hide: false },
                        ]
                          .filter(({ hide }) => !hide)
                          .map(({ label, sx, align }) => (
                          <TableCell
                    ...
                    <TableBody>
                      {filteredBatchHistory.map((his, index) => (
                        ...
                        >
                          ...
                          <TableCell align="center">
                            <Tooltip title="실행 로그 보기">
                              <span>
                                <IconButton
                                  size="small"
                                  onClick={() => handleViewResult(his.bat_exe_log_sn, his.rslt_txt)}
                                  disabled={!his.has_rslt_txt && !his.rslt_txt}
                                  aria-label="로그 보기"
                                >
